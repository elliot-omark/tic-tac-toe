...
public class UI
{
    ...
    public void printBoard(State state) {
        ...
        for (int row = 1; row <= Constants.BOARD_SIZE; row++) {
            System.out.printf(Constants.BOARD_STRING, getXOrO(state.getBoardCell(row, 1)), getXOrO(state.getBoardCell(row, 2)), getXOrO(state.getBoardCell(row, 3)));
            System.out.println();
            System.out.println(Constants.DIVIDER_STRING);
    ...
    public void printMove(State state, int row, int col) {
        System.out.printf(Constants.PRINT_MOVE, getXOrO(state.getWhoseMove()), this.getPlayerName(state.getWhoseMove(), state.getXName(), state.getOName()), row, col);
    } 

    public void printWinner(State state) {
        System.out.printf(Constants.WINNER, getXOrO(state.getWhoseMove()), this.getPlayerName(state.getWhoseMove(), state.getXName(), state.getOName()));
    }
